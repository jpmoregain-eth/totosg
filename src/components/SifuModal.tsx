...
export default function SifuModal({ visible, onClose, lang, sifuUnlocked, onUnlock }: Props) {
  ...
  useEffect(() => {
    ...
    const unsubClosed = rewarded.addAdEventListener(AdEventType.CLOSED, () => {
      setAdLoaded(false);
      rewarded.load();
      // If ad closed without reward (crashed/skipped by system) — still proceed as grace fallback
      if (!rewardEarned.current) { onUnlock(); showResults(); }
  ...
  const handleConsult = () => { if (sifuUnlocked) showResults(); else setScreen('adprompt'); };
  const handleWatchAd = () => { if (adLoaded) rewarded.show(); else { onUnlock(); showResults(); } };
