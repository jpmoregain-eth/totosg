...
function GofDialog({ visible, onClose, lang }) {
  return (
      ...
      <TouchableOpacity style={gofStyles.overlay} activeOpacity={1} onPress={onClose}>
        <View style={gofStyles.dialog}>
          <Image
            ...
            style={gofStyles.dialogImage}
            resizeMode="contain"
          />
          <Text style={gofStyles.dialogTitle}>
            {lang === 'ZH' ? '财神到！🧧' : '财神到！🧧'}
          </Text>
          <Text style={gofStyles.dialogMsg}>
            {lang === 'ZH'
              ? '有缘人，你好！恭喜发财，万事如意！\n\n财神的玄学号码生成器即将在八月中秋节前降临，以天地之力助你开彩！感谢您一直以来的支持，好运即将降临！🎰'
              : 'Greetings, fortunate one! Wishing you prosperity and good fortune!\n\nThe God of Fortune\'s mystical number generator is arriving in August, just in time for the Mid-Autumn Festival 🏮 to bless your lottery picks with cosmic alignment. Thank you for your support — your luck is about to change! 🎰'}
          </Text>
          <TouchableOpacity style={gofStyles.closeBtn} onPress={onClose}>
            <Text style={gofStyles.closeBtnText}>
              {lang === 'ZH' ? '发财！🧧 关闭' : '发财！🧧 Close'}
            </Text>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
...
function GofFAB({ lang }) {
  const [dialogVisible, setDialogVisible] = useState(false);
  const [showHint, setShowHint] = useState(true);
  const hintOpacity = useRef(new Animated.Value(1)).current;
  const pan = useRef(new Animated.ValueXY({
    x: SCREEN_W / 2 - FAB_SIZE / 2,
    y: SCREEN_H / 2 - FAB_SIZE / 2,
  })).current;
  const isDragging = useRef(false);
  const lastPos = useRef({ x: SCREEN_W / 2 - FAB_SIZE / 2, y: SCREEN_H / 2 - FAB_SIZE / 2 });
  ...
  const panResponder = useRef(PanResponder.create({
    ...
    onPanResponderRelease: (_, gs) => {
      ...
      const snapX = currentX + FAB_SIZE / 2 < SCREEN_W / 2
        ? 16
        : SCREEN_W - FAB_SIZE - 16;
      ...
      if (!isDragging.current) setDialogVisible(true);
    },
  })).current;
  ...
  return (
    <>
      <Animated.View
        style={[
          gofStyles.fab,
          {
            transform: [
              { translateX: pan.x },
              { translateY: Animated.add(pan.y, bobAnim) },
            ],
          },
        ]}
      ...
      <GofDialog visible={dialogVisible} onClose={() => setDialogVisible(false)} lang={lang} />
    </>
  );
}
...
function MainApp() {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* GoF FAB — rendered outside SafeAreaView so it floats over everything */}
      <GofFAB lang={lang} />
    </View>
  );
}
